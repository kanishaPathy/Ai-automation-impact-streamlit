...
import joblib  # Assuming you're loading a pre-trained model using joblib
...
# Use the correct column name for Education Level
education_level = st.sidebar.selectbox("Education Level", sorted(df['_id.EducationLevel'].unique()))  # Updated column name
...
input_df = pd.DataFrame({
    ...
    '_id.Sector': [sector],
    '_id.Year': [year],
    '_id.EducationLevel': [education_level],  # Updated to include Education Level
    'Avg_PreAI': [pre_ai],
    'Avg_PostAI': [post_ai],
...
# Load your trained model (make sure the model is saved as a joblib file)
model = joblib.load("xgboost_model.pkl")
  # Replace with actual path to your trained model
