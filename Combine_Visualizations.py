...
# Gender vs Sector Visualization (from df2)
st.markdown("---")
st.header("👩‍💻 Gender Representation in Sectors")
if 'Gender' in df2.columns and 'Sector' in df2.columns:
    gender_sector = df2.groupby(['Gender', 'Sector']).size().reset_index(name='Count')
    fig6 = px.bar(gender_sector, x='Sector', y='Count', color='Gender', barmode='group',
                  title='Gender Distribution Across Sectors')
    st.plotly_chart(fig6, use_container_width=True)
else:
    st.warning("Gender or Sector column not found in reskilling dataset.")

# Skill Level vs Pre/Post AI
st.markdown("---")
st.header("🛠️ Skill Level Impact (Pre vs Post AI)")
if 'Skill_Level' in df2.columns and 'Avg_PreAI' in df2.columns and 'Avg_PostAI' in df2.columns:
    skill_impact = df2.groupby('Skill_Level')[['Avg_PreAI', 'Avg_PostAI']].mean().reset_index()
    fig7 = px.bar(skill_impact, x='Skill_Level', y=['Avg_PreAI', 'Avg_PostAI'],
                  barmode='group', title='Skill Level vs Pre/Post AI Impact')
    st.plotly_chart(fig7, use_container_width=True)
else:
    st.warning("Skill_Level or impact columns not found in reskilling dataset.")
